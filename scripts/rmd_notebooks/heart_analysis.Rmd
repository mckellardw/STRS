...
```{r warning=FALSE}
# collapse multimappers
## Deprecated - multimapper collapsing issue fixed above!
heart.list <- lapply(
  heart.list,
  FUN = function(SEU) collapseMultimappers(
    SEU,
...
tmp.mat <- tmp.mat[reo.ids, colnames(tmp.mat) %in% Cells(heart.list[[2]])] #only keep Reo genes and spots that pass QC
...
tmp.mat <- tmp.mat[reo.ids, Cells(heart.list[[2]])] #make sure everything is in the right order
...
tmp.mat <- tmp.mat[reo.ids, colnames(tmp.mat) %in% Cells(heart.list[[4]])] #only keep Reo genes and spots that pass QC
