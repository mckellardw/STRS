...
for(i in 1:nrow(meta_heart)){
  ...
  tmp.seu <- ReadMtx(
    mtx=paste0(meta_heart$data.dir.kallisto.REO[i],"/kb_standard/counts_unfiltered/matrix.mtx"),
    cells=paste0(meta_heart$data.dir.kallisto.REO[i],"/kb_standard/counts_unfiltered/barcodes.tsv"),
    features=paste0(meta_heart$data.dir.kallisto.REO[i],"/kb_standard/counts_unfiltered/genes.tsv"),
    feature.column=1,
    mtx.transpose = F
  ) %>%
    CreateSeuratObject() %>%
...
for(i in 1:length(heart.list)){
  ...
  tmp.seu <- ReadMtx(
    mtx=paste0(meta_heart$data.dir.kallisto[i],"/kb_miRNA/counts_unfiltered/matrix.mtx"),
    cells=paste0(meta_heart$data.dir.kallisto[i],"/kb_miRNA/counts_unfiltered/barcodes.tsv"),
    features=paste0(meta_heart$data.dir.kallisto[i],"/kb_miRNA/counts_unfiltered/genes.tsv"),
    feature.column=1,
    mtx.transpose = F
  ) %>%
    CreateSeuratObject() %>%
...
for(i in 1:length(xGen.samples)){
  ...
  tmp.seu <- ReadMtx(
    mtx=paste0(xGen.samples[i],"/kb_standard/counts_unfiltered/matrix.mtx"),
    cells=paste0(xGen.samples[i],"/kb_standard/counts_unfiltered/barcodes.tsv"),
    features=paste0(xGen.samples[i],"/kb_standard/counts_unfiltered/genes.tsv"),
    feature.column=1,
    mtx.transpose = F #*note- switch to `T` for kb_python outputs
  ) %>%
    CreateSeuratObject() 
...
heart.list[[2]]$nCount_xGen.kallisto.as <- tmp.mat[tmp.feat[grep(tmp.feat,pattern="-as")],] %>% colSums()
