...
for(i in 1:length(xGen.samples)){
  ...
  tmp.seu <- ReadMtx(
    mtx=paste0(xGen.samples[i],"/kb_standard/counts_unfiltered/cells_x_genes.mtx"),
    cells=paste0(xGen.samples[i],"/kb_standard/counts_unfiltered/cells_x_genes.barcodes.txt"),
    features=paste0(xGen.samples[i],"/kb_standard/counts_unfiltered/cells_x_genes.genes.txt"),
    feature.column=1,
    mtx.transpose = T
  ) %>%
    CreateSeuratObject() 
