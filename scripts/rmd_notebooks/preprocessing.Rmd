...
for(i in 1:nrow(meta_vis)){
  #TODO- update this with faster code
  tmp.h5 <- paste0(meta_vis$data.dir.kallisto[i],"/kb_standard/counts_unfiltered/adata")
...
for(i in 1:length(vis.list)){
  ...
  vis.list[[i]]$pct_kallisto_spliced <- vis.list[[i]]$nCount_kallisto_spliced/vis.list[[i]]$nCount_kallisto*100
  ...
  vis.list[[i]]$pct_kallisto_unspliced <- vis.list[[i]]$nCount_kallisto_unspliced/vis.list[[i]]$nCount_kallisto*100
...
save(
  vis.list,
  file="/workdir/dwm269/totalRNA/spTotal/robjs/vis_list_v3.RData"
)
```
...
tmp.feat <- paste0("pct.",unique(gtf.info$gene_biotype))
