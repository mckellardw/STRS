...
### Read in GENCODE polyA features
From GENCODE's README:
...
```{r}
#TODO - get transcript IDs to gene symbol IDs converted
pa.info <- read.table("/local/workdir/dwm269/totalRNA/spTotal/resources/gene_lists/GENCODEM28_polyA_features.tsv")
colnames(pa.info) <- c(
  "transcript_id",
  "V2","V3",
  "chr", "Start", "stop",
...
# Remove period/decimal suffixes from ensembl IDs
pa.info$GeneID <- strsplit(
  x=pa.info$transcript_id,
  split = "\\."
)%>%
  lapply(
    FUN=function(X) X[[1]]
...
pa.info$GeneSymbol <- ens2gene(
  pa.info$transcript_id,
