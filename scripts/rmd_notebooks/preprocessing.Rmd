...
for(i in 1){#:nrow(meta_vis)
  ...
  if(!file.exists(paste0(tmp.h5,".h5seurat"))){
    message(paste0("Converting h5seurat for",meta_vis$capture.area,"...\n"))
    Convert(
      paste0(tmp.h5,".h5ad"),
      dest = "h5seurat",
      overwrite = TRUE
  ...
  vis.list[[i]][["kallisto"]] <- CreateAssayObject(
    counts = GetAssayData(tmp.seu,assay="RNA"),
    min.cells = 1
  )
...
# }
```
### Collapse multimappers, preprocess collapsed counts (kallisto)
```{r warning=FALSE}
# vis.list <- lapply(
#   vis.list,
#   FUN = function(SEU) collapseMultimappers(
