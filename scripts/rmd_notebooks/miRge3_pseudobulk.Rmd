...
rownames(mir.rpm)[grep(rownames(mir.rpm), pattern="451")]

tmp.df = data.frame(meta_smRNA[18:36,c("chemistry","polyA")])
rownames(tmp.df)<-meta_smRNA$sample[18:36]

pheatmap::pheatmap(
  mir.rpm[tmp.feat, 18:36],
  annotation_col = tmp.df,
  border_color = "black",
  cluster_rows = F,
  cluster_cols = F
